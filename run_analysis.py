...
# Load the semantic search model for ranking chunks (offline/local)
print("Loading semantic search model (all-MiniLM-L6-v2) from local directory...")
ranking_model = SentenceTransformer(os.path.join("models", "all_MiniLM_L6_v2"))

# Load the generative model for creating titles (offline/local, using flan-t5-small)
print("Loading title generation model (google/flan-t5-small) from local directory...")
title_tokenizer = T5Tokenizer.from_pretrained(os.path.join("models", "flan_t5_small"))
title_model = T5ForConditionalGeneration.from_pretrained(os.path.join("models", "flan_t5_small"))
